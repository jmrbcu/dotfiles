...
def install_ohmyzsh():
    ...
    try:
        output = check_output(shlex.split(cmd), shell=True)
        print yellow(output)
        return True
    except CalledProcessError as e:
        print '{0}\n{1}'.format(red(e), yellow(e.output))
        return False
...
def link_files(home):
    ...
    for filename in os.listdir(current):
        ...
        if os.path.exists(full_filename):
            print 'Configuration file exists: {0}, backing it up to: {1}'.format(full_filename, full_filename + '.save')
            shutil.move(full_filename, full_filename + '.save')

        print yellow('Linking file: {0}'.format(blue(os.path.join(current, filename))))
